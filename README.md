...
## Overview
This capstone project leverages Fiserv’s Small Business Index (FSBI) and related spending trend data to forecast U.S. Personal Consumption Expenditures (PCE), a key economic indicator. We then simulate trading strategies on a FORECASTEX-style framework to model potential gains/losses from more accurate PCE predictions. :contentReference[oaicite:1]{index=1}

## Goals
1. Establish whether FSBI is a good leading predictor of the PCE index.  
2. If yes, identify and simulate profitable trading strategies using this information. :contentReference[oaicite:2]{index=2}
...
- Align datasets across a consistent timeframe/frequency (monthly or quarterly). :contentReference[oaicite:3]{index=3}
...
- Track model performance with metrics such as RMSE, MAPE, MAE, R², and cross-validation where appropriate. :contentReference[oaicite:4]{index=4}
...
- Evaluate results with ROI, Sharpe ratio, maximum drawdown, and related diagnostics. :contentReference[oaicite:5]{index=5}
...
- Provide actionable insights and recommendations based on findings. :contentReference[oaicite:6]{index=6}

### 5) Final Report and Presentation
- Document the end-to-end process, including data, modeling, trading simulation, and visualizations.
- Summarize findings, limitations, and recommendations. :contentReference[oaicite:7]{index=7}

## Key Deliverables
1. **Preprocessed Dataset**
   - Integrated clean dataset combining FSBI with PCE and relevant indicators
   - Data transformation scripts and feature engineering documentation :contentReference[oaicite:8]{index=8}

2. **PCE Forecasting Model**
   - Trained forecasting model and evaluation results (RMSE, MAPE, MAE, R², cross-validation)
   - Documentation of model selection rationale :contentReference[oaicite:9]{index=9}
...
3. **Trading Simulation Framework**
   ...
   - Analysis under varying market conditions :contentReference[oaicite:10]{index=10}

4. **Interactive Visualizations**
   - Dashboard(s) showing predictions, drivers, and trading outcomes
   - Supporting charts/figures for analysis and reporting :contentReference[oaicite:11]{index=11}

5. **Final Report**
   - Comprehensive methodology, results, trading outcomes, and practical implications :contentReference[oaicite:12]{index=12}

## Data Sources
This project is based solely on publicly available data plus the provided Fiserv datasets. :contentReference[oaicite:13]{index=13}
...
- Libraries/Tooling (as needed):
  ...
  - Trading simulation: Backtrader, Monte Carlo simulation :contentReference[oaicite:14]{index=14}
...
- **Week 15**: Final Report & Presentation :contentReference[oaicite:15]{index=15}
